...
class TestAccountingService:
    ...
    def test_reset_and_seed_agency_accounts_deletes_existing_accounts_and_entries(
        self, sqlite_repo
    ):
        with patch.object(AccountingService, "_ensure_default_accounts_exist"):
            service = AccountingService(sqlite_repo, EventBus())
        ...
        with patch.object(
            service, "seed_default_accounts", return_value={"created": 0, "failed": 0}
        ):
            result = service.reset_and_seed_agency_accounts()

        assert result == {"created": 0, "failed": 0}
        assert sqlite_repo.get_all_accounts() == []
