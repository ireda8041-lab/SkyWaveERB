...
class PDFGenerator:
    ...
    def _create_ledger_header(self, account: schemas.Account, date_range: Dict[str, datetime]) -> Table:
        ...
        header_data = [
            [f"Account Statement", "", f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}"],
...
class PrintingService:
    ...
    def _html_to_pdf(self, html_content: str, filename_prefix: str, client_phone: str = None) -> Optional[str]:
        ...
        try:
            ...
            print(f"WARNING: [PrintingService] Failed to create PDF. Saving as HTML instead.")
            print(f"INFO: To generate PDF, please install one of the following:")
            print(f"   1. wkhtmltopdf: https://wkhtmltopdf.org/downloads.html")
            print(f"   2. Google Chrome or Microsoft Edge")
...
class TemplateManager:
    ...
    def preview_template(self, template_id: str, sample_data: Dict[str, Any] = None) -> Optional[str]:
