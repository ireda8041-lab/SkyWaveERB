...
class UnifiedHRManager(QWidget):
    ...
    def _create_employees_tab(self):
        ...
        self.emp_info_label.setStyleSheet(
            "background-color: #1e3a8a; color: white; padding: 10px; border-radius: 6px; font-weight: bold;"
        )
    ...
    def _update_employees_table(self):
        ...
        for row, emp in enumerate(self.employees):
            ...
            self.employees_table.setItem(
                row, 5, create_centered_item(f"{emp.get('salary', 0):.2f}")
            )
    ...
    def _create_loans_tab(self):
        ...
        self.loans_emp_info.setStyleSheet(
            "font-weight: bold; color: #4a90e2; padding: 10px; background-color: #002040; border-radius: 6px;"
        )
        ...
        self.loans_summary_label.setStyleSheet(
    ...
    def _update_loans_table(self, loans):
        ...
        for row, loan in enumerate(loans):
            ...
            self.loans_table.setItem(
                row, 4, create_centered_item(f"{loan.get('remaining_amount', 0):.2f}")
            )
            self.loans_table.setItem(
                row, 5, create_centered_item(f"{loan.get('monthly_deduction', 0):.2f}")
            )
            ...
            self.loans_table.setItem(
                row, 6, create_centered_item(f"{paid_installments}/{total_installments}")
            )
    ...
    def _create_salaries_tab(self):
        ...
        self.salaries_summary_label.setStyleSheet(
            "background-color: #1e3a8a; color: white; padding: 10px; border-radius: 6px;"
        )
    ...
    def _load_salaries(self):
        ...
        for row, sal in enumerate(salaries):
            ...
            self.salaries_table.setItem(
                row, 2, create_centered_item(f"{sal.get('basic_salary', 0):.2f}")
            )
            self.salaries_table.setItem(
                row, 3, create_centered_item(f"{sal.get('allowances', 0):.2f}")
            )
            self.salaries_table.setItem(
                row, 4, create_centered_item(f"{sal.get('bonuses', 0):.2f}")
            )
            self.salaries_table.setItem(
                row, 5, create_centered_item(f"{sal.get('overtime_amount', 0):.2f}")
            )
            self.salaries_table.setItem(
                row, 6, create_centered_item(f"{sal.get('loan_deductions', 0):.2f}")
            )
            self.salaries_table.setItem(
                row, 7, create_centered_item(f"{sal.get('insurance_deduction', 0):.2f}")
            )
            self.salaries_table.setItem(
                row, 8, create_centered_item(f"{sal.get('tax_deduction', 0):.2f}")
            )

            total_ded = (
                sal.get("loan_deductions", 0)
                + sal.get("insurance_deduction", 0)
                + sal.get("tax_deduction", 0)
                + sal.get("other_deductions", 0)
            )
...
"""
    ...
    def _update_attendance_table(self, attendance):
        ...
        for row, att in enumerate(attendance):
            ...
            self.attendance_table.setItem(
                row, 1, create_centered_item(att.get("employee_name", ""))
            )
            self.attendance_table.setItem(row, 2, create_centered_item(att.get("department", "")))
            self.attendance_table.setItem(
                row, 3, create_centered_item(att.get("check_in_time", ""))
            )
            self.attendance_table.setItem(
                row, 4, create_centered_item(att.get("check_out_time", ""))
            )
            self.attendance_table.setItem(
                row, 5, create_centered_item(f"{att.get('work_hours', 0):.1f}")
            )
            self.attendance_table.setItem(
                row, 6, create_centered_item(f"{att.get('overtime_hours', 0):.1f}")
            )
    ...
    def check_in(self):
        ...
        if success:
            ...
            QMessageBox.information(
                self, "✅ نجح", f"تم تسجيل حضور {self.current_employee['name']}"
            )
        else:
            QMessageBox.critical(self, "❌ خطأ", msg)
    ...
    def check_out(self):
        ...
        if success:
            self._load_today_attendance()
            QMessageBox.information(
                self, "✅ نجح", f"تم تسجيل انصراف {self.current_employee['name']}"
            )
        else:
            QMessageBox.critical(self, "❌ خطأ", msg)
...
class LoanDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        # معلومات الموظف
        emp_info = QLabel(
            f"👤 الموظف: {self.employee['name']} - راتب: {self.employee.get('salary', 0):.2f} ج.م"
        )
        emp_info.setStyleSheet(
            "font-weight: bold; color: #4a90e2; padding: 10px; background-color: #002040; border-radius: 6px;"
        )
        ...
        self.loan_type.addItems(
            ["سلفة شخصية", "قرض إسكان", "مقدم راتب", "سلفة طوارئ", "قرض سيارة", "أخرى"]
        )
        ...
        note.setStyleSheet(
            "background-color: #1e3a8a; color: white; padding: 8px; border-radius: 4px; font-size: 11px;"
        )
...
class PayLoanDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        info = QLabel(
            f"المبلغ المتبقي: {self.remaining:.2f} ج.م\nالقسط الشهري: {self.monthly:.2f} ج.م"
        )
        info.setStyleSheet(
            "font-weight: bold; color: #4a90e2; padding: 10px; background-color: #002040; border-radius: 6px;"
        )
    ...
    def _pay(self):
        method_map = {
            "خصم من الراتب": "salary_deduction",
            "نقدي (كاش)": "cash",
            "تحويل بنكي": "bank",
        }
        method = method_map.get(self.method.currentText(), "salary_deduction")

        success, msg = self.hr_service.pay_loan_installment(
            self.loan_id, self.amount.value(), method
        )
...
class LoanDetailsDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        self.progress_label.setStyleSheet(
            "font-size: 14px; font-weight: bold; color: #4a90e2; padding: 10px;"
        )
    ...
    def _load_data(self):
        ...
        for loan_item in loans:
            if loan_item.get("id") == self.loan_id:
                loan = loan_item
                break
        ...
        self.progress_label.setText(
            f"📊 التقدم: {progress:.1f}% ({amount - remaining:,.2f} من {amount:,.2f} ج.م)"
        )
        ...
        for row, payment in enumerate(payments):
            self.payments_table.setItem(
                row, 0, create_centered_item(payment.get("payment_date", ""))
            )
            self.payments_table.setItem(
                row, 1, create_centered_item(f"{payment.get('amount', 0):,.2f} ج.م")
            )
...
class EditSalaryDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        self.summary_label.setStyleSheet(
            "font-size: 16px; font-weight: bold; color: #22c55e; padding: 15px; background-color: #002040; border-radius: 6px; text-align: center;"
        )
    ...
    def _load_data(self):
        ...
        if employee:
            ...
            self.emp_info.setText(
                f"👤 {employee['name']} | الراتب الأساسي: {self.basic_salary:,.2f} ج.م"
            )
        else:
            self.basic_salary = 0
