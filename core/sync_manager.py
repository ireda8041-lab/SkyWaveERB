...
from .schemas import SyncOperation, SyncPriority, SyncQueueItem, SyncStatus
...
class SyncManager:
    ...
    def _get_entity_data(self, entity_type: str, entity_id: str) -> dict[str, Any] | None:
        """الحصول على بيانات الكيان من SQLite"""
        try:
            # التحقق من صحة اسم الجدول
            if not self._validate_table_name(entity_type):
                logger.error(f"اسم جدول غير صالح: {entity_type}")
                return None
    ...
    def _get_mongo_id(self, entity_type: str, entity_id: str) -> str | None:
