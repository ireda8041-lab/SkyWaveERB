        ...
        try:
            ...
            try:
                ...
                for entry in journal_entries:
                    try:
                        ...
                        mongo_id = str(entry_dict.pop('_id'))

                        # تحويل datetime
                        for key in ['created_at', 'last_modified', 'date']:
                            if key in entry_dict and hasattr(entry_dict[key], 'isoformat'):
                                entry_dict[key] = entry_dict[key].isoformat()

                        # تحويل lines إلى JSON
                        lines_json = json.dumps(entry_dict.get('lines', []))
                        ...
                        """, (
                            ...
                            lines_json,
                            entry_dict.get('related_document_id'),
                            entry_dict.get('created_at'),
                            entry_dict.get('last_modified'),
                        ))
                        entries_pulled += 1
                    except Exception as ex:
            ...
            try:
                ...
                for exp in expenses:
                    try:
                        exp_dict = dict(exp)
                        mongo_id = str(exp_dict.pop('_id'))

                        # تحويل datetime
                        for key in ['created_at', 'last_modified', 'date']:
                            if key in exp_dict and hasattr(exp_dict[key], 'isoformat'):
                        ...
                        """, (
                            ...
                            exp_dict.get('amount', 0.0),
                            exp_dict.get('description'),
                            exp_dict.get('account_id'),
                            exp_dict.get('payment_account_id'),
                            exp_dict.get('project_id'),
                            exp_dict.get('created_at'),
                            exp_dict.get('last_modified'),
                        ))
                        expenses_pulled += 1
                    except Exception as ex:
    ...
    def _push_to_mongo(self) -> int:
        ...
        for table_name, config in tables_config.items():
            ...
            if not self._validate_table_name(table_name):
                print(f"  ⚠️ اسم جدول غير صالح: {table_name}")
                continue
