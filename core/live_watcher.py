...
class LiveDataWatcher(QObject):
    ...
    WATCHED_TABLES = ["clients", "projects", "payments", "expenses", "accounts"]
    ...
    def start(self):
        ...
        logger.info("[LiveWatcher] 🚀 بدء المراقبة كل %s ثواني", self.check_interval)
    ...
    def _init_counts(self):
        ...
        try:
            ...
            logger.debug("[LiveWatcher] خطأ في تهيئة الأعداد: %s", e)
    ...
    def _check_local_changes(self):
        ...
        try:
            ...
            try:
                for table in self.WATCHED_TABLES:
                    try:
                        ...
                        if current_count != self._last_counts.get(
                            table, 0
                        ) or current_modified != self._last_modified.get(table):
            ...
            logger.debug("[LiveWatcher] خطأ في فحص التغييرات: %s", e)
    ...
    def _emit_pending_changes(self):
        ...
        for table in tables:
            logger.debug("[LiveWatcher] 📢 تغيير في %s", table)
            try:
    ...
    def _start_mongo_watcher(self):
        ...
        def watch_mongo():
            ...
            while not self._stop_event.is_set() and not self._shutdown:
                try:
                    ...
                    for table in self.WATCHED_TABLES:
                        ...
                        try:
                            ...
                            with collection.watch(
                                full_document="updateLookup", max_await_time_ms=2000
                            ) as stream:
                                for change in stream:
                                    ...
                                    logger.info(
                                        "[LiveWatcher] 🔴 MongoDB: %s في %s", operation, table
                                    )
                                    safe_print(
                                        f"INFO: [LiveWatcher] 🔴 MongoDB: {operation} في {table}"
                                    )
                            ...
                            if "timed out" not in str(e).lower():
                                logger.debug("[LiveWatcher] خطأ في مراقبة %s: %s", table, e)
                    ...
                    if self._shutdown:
                        break
                    logger.debug("[LiveWatcher] خطأ في MongoDB watcher: %s", e)
                    time.sleep(5)
        ...
        self._mongo_watcher_thread = threading.Thread(
            target=watch_mongo, daemon=True, name="MongoWatcher"
        )
...
class LiveUpdateRouter(QObject):
    ...
    def handle_data_change(self, table_name: str):
        ...
        """
        logger.debug("[LiveRouter] 📢 تغيير في: %s", table_name)
        ...
        try:
            ...
            logger.debug("[LiveRouter] خطأ في توجيه التحديث: %s", e)
    ...
    def _refresh_table_fallback(self, table_name: str):
        ...
        try:
            ...
            logger.debug("[LiveRouter] خطأ في تحديث الواجهة: %s", e)
    ...
    def handle_sync_needed(self, table_name: str):
        ...
        """
        logger.info("[LiveRouter] 🔄 مزامنة مطلوبة: %s", table_name)
